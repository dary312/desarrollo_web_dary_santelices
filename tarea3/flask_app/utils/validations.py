...
def validate_comentario_nombre(value):
    if value is None or value.strip() == "":
        return False
    return len(value.strip()) >= 3 and len(value.strip()) <= 80


def validate_comentario_texto(value):
    if value is None or value.strip() == "":
        return False
    return len(value.strip()) >= 5 and len(value.strip()) <= 200